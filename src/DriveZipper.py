...
class GUI(object):
    ...
    def __init__(self, root):
        ...
        self.zip_dir_button = customtkinter.CTkButton(master=root, text="...", width=75, command=lambda: self.ask_for_file(self.zip_entry))
    ...
    def submit(self):
        ...
        self.progresslabel.place(relx=0.5, rely=0.82, anchor=tkinter.CENTER)
        ...
        if root.getvar(name="zip"):
            ...
            if root.getvar(name="download"):
                ...
                tzip = threading.Thread(target=zip, args=(os.path.join(os.getcwd(), "download", torrent_info.name), self.zip_entry.get(), self.zip_option.get(), zipped))
                tzip.start()
            else:
                #zipped = zip(self.download_entry.get(), self.zip_entry.get(), self.zip_option.get())
                tzip = threading.Thread(target=zip, args=(self.download_entry.get(), self.zip_entry.get(), self.zip_option.get(), zipped))
                tzip.start()
            while tzip.is_alive():
                root.update()
            print("done zipping!")
        ...
        if root.getvar(name="upload"):
            ...
            if root.getvar(name="zip"):
                #drive.uploadFile(zipped["name"], self.folders[self.upload_dropdown.get()], zipped["dir"])
                tupload = threading.Thread(target=drive.uploadFile, args=(zipped["name"], self.folders[self.upload_dropdown.get()], zipped["dir"]))
                tupload.start()
            else:
                zipname = self.zip_entry.get().split("/")[-1]
                print(zipname)
                #drive.uploadFile(zipname, self.folders[self.upload_dropdown.get()], self.zip_entry.get())
                tupload = threading.Thread(target=drive.uploadFile, args=(zipname, self.folders[self.upload_dropdown.get()], self.zip_entry.get()))
                tupload.start()
            while tupload.is_alive():
                root.update()
        ...
        elif root.getvar(name="remove_torrent"):
            self.progresslabel.configure(text="DELETING...")
            self.progressbar.set(0.9)
            remove_torrent(False, get_torrent_hash())
        if root.getvar(name="delete_zip"):
            self.progresslabel.configure(text="DELETING...")
            self.progressbar.set(0.9)
            if root.getvar(name="zip"):
                os.remove(zipped["dir"])
            else:
                os.remove(self.zip_entry.get())
